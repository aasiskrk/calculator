#need to import tkinter package and lambda
from _ast import Lambda
from tkinter import *


#creating a root so we can view the wigets in window

root= Tk()


#basic gui modification

root.iconbitmap("calculator.ico")
root.title("Simple calculator")
root.configure(bg="light blue")
# root.geometry("350x432")

#creating entry box
e = Entry(root, width=50, borderwidth=5)
e.grid(row=0,column=0,columnspan=3, padx=10,pady=10)


#functions for calculating each operations
def button_click(number):
    current = e.get()
    e.delete(0,END)
    e.insert(0,str(current)+str(number))

def button_clear():
    e.delete(0, END)

def button_add():
    first_number = e.get()
    global f_num,a
    f_num = int(first_number)
    e.delete(0,END) 
    a = 1

def button_minus():
    first_number = e.get()
    global f_num,a
    f_num=int(first_number)
    e.delete(0,END)
    a = 2

def button_mul():
    first_number= e.get()
    global f_num,a
    f_num=int(first_number)
    e.delete(0,END)
    a =3

def button_div():
    first_number=e.get()
    global f_num,a
    f_num=int(first_number)
    e.delete(0,END)
    a = 4

def button_rem():
    first_number=e.get()
    global f_num,a
    f_num=int(first_number)
    e.delete(0,END)
    a = 5

#function after operation is selected and equal button is clicked
def button_equal():
    second_number = e.get()
    e.delete(0,END)
    if a ==1:
         e.insert(0, f_num + int(second_number))
    elif a ==2:
         e.insert(0, f_num - int(second_number))
    elif a == 3:
         e.insert(0, f_num * int(second_number))
    elif a == 4:
         e.insert(0, f_num / int(second_number))
    elif a == 5:
         e.insert(0, f_num % int(second_number))
        
#creating buttons for the calculator

button_1= Button(root, text="1",padx=50 ,pady=20, command = lambda:button_click(1))
button_2= Button(root, text="2",padx=50 ,pady=20, command = lambda:button_click(2))
button_3= Button(root, text="3",padx=50 ,pady=20, command = lambda:button_click(3))
button_4= Button(root, text="4",padx=50 ,pady=20, command = lambda:button_click(4))
button_5= Button(root, text="5",padx=50 ,pady=20, command = lambda:button_click(5))
button_6= Button(root, text="6",padx=50 ,pady=20, command = lambda:button_click(6))
button_7= Button(root, text="7",padx=50 ,pady=20, command = lambda:button_click(7))
button_8= Button(root, text="8",padx=50 ,pady=20, command = lambda:button_click(8))
button_9= Button(root, text="9",padx=50 ,pady=20, command = lambda:button_click(9))
button_0= Button(root, text="0",padx=50 ,pady=20, command = lambda:button_click(0))

button_add= Button(root,text="+",padx=48,pady=20,command= button_add)

button_minus=Button(root,text="-",padx=50,pady=20,command=button_minus)

button_mul=Button(root,text="*",padx=51,pady=20,command=button_mul)

button_div=Button(root,text="/",padx=50,pady=20,command=button_div)

button_rem= Button(root,text="%",padx=50,pady=20,command=button_rem)

button_equal=Button(root,text="=",padx=48,pady=20,command=button_equal)

button_clear=Button(root,text="Clear",padx=98,pady=20,command=button_clear)


#placing the buttons using grid method on screen
button_1.grid(row=3,column=0)
button_2.grid(row=3,column=1)
button_3.grid(row=3,column=2)


button_4.grid(row=2,column=0)
button_5.grid(row=2,column=1)
button_6.grid(row=2,column=2)


button_7.grid(row=1,column=0)
button_8.grid(row=1,column=1)
button_9.grid(row=1,column=2)


button_0.grid(row=4,column=0)

button_clear.grid(row=5,column=1,columnspan=2)

button_add.grid(row=5,column=0)

button_minus.grid(row=6,column=0)

button_mul.grid(row=4,column=1)

button_div.grid(row=4,column=2)

button_rem.grid(row=6,column=1)

button_equal.grid(row=6,column=2)


#closing the root 
root.mainloop()